...
const emit = defineEmits(['add-publisher']);
...
const fetchSuggestions = debounce(async (query) => {
    ...
    try {
        const response = await axios.get(route('publisher.search'), {
            params: { name: query },
        });
...
<template>
    <div class="relative">
        <input type="text" v-model="search" @input="onInput" placeholder="Rechercher un éditeur"
        ...
        <ul v-if="suggestions.length && showSuggestions"
            ...
            <li v-for="publisher in suggestions" :key="publisher.id" @click="selectPublisher(publisher)"
