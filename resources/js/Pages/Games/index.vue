        ...
        <div class="py-3">
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <div class="relative w-full max-w-md mx-auto">
                    <input type="text" v-model="searchQuery" @input="debouncedSearch" placeholder="Recherchez par nom"
                        class="border rounded p-2 w-full pr-10 my-5" />
                    <button v-if="searchQuery" @click="clearInput"
                ...
                <div v-if="!isLoading" class="mt-6 flex justify-center items-center space-x-4">
                    <button id="fetchprev" @click="fetchGames(games.prev_page_url)" v-if="games.prev_page_url"
                        class="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300 transition">
                        &lt; Previous
                    </button>
                    <span>Page {{ games.current_page }} of {{ games.last_page }}</span>
                    <button id="fetchnext" @click="fetchGames(games.next_page_url)" v-if="games.next_page_url"
                        class="px-4 py-2 bg-gray-200 rounded hover:bg-gray-300 transition">
                        Next &gt;
                    </button>
                </div>
